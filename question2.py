...
def load_data(attach1: Path, attach2: Path) -> tuple[np.ndarray, pd.DataFrame, pd.DataFrame]:
    for path in (attach1, attach2):
        if not path.is_file():
            raise FileNotFoundError(f"未找到输入文件：{path}（应位于 question2.py 所在仓库根目录）")
...
def template_interval_label(t: int) -> str:
    """严格复现附件5表头：0:10起，跨日终点以“+1”标识。"""
    start = (t + 1) * 10
    end = start + 10
    start_text = "0:00" if start == 1440 else f"{(start % 1440) // 60}:{(start % 60):02d}"
    if end >= 1440:
        end_text = f"{(end % 1440) // 60}:{(end % 60):02d}+1"
    else:
        end_text = f"{end // 60}:{end % 60:02d}"
    return f"{start_text}-{end_text}"


def export_result(detail: pd.DataFrame, output: Path) -> None:
    # 附件5“计划购电量”模板采用0:10起的显示标签，并要求两个全天汇总列。
    template_slots = [template_interval_label(t) for t in range(T)]
